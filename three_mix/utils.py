...
def GenIdx( train_color_label,train_rgbir_label, train_thermal_label):
    ...
    rgbir_pos = []
    unique_label_color = np.unique(train_rgbir_label)
    for i in range(len(unique_label_color)):
        tmp_pos = [k for k, v in enumerate(train_rgbir_label) if v == unique_label_color[i]]
        rgbir_pos.append(tmp_pos)
    ...
    return color_pos, rgbir_pos, thermal_pos
...
class IdentitySampler(Sampler):
    ...
    def __init__(self, train_color_label, train_rgbir_label, train_thermal_label, color_pos, rgbir_pos, thermal_pos,
                 num_pos, batchSize, epoch):
        ...
        for j in range(int(N / (batchSize * num_pos)) + 1):
            ...
            for i in range(batchSize):
                ...
                sample_rgbir = np.random.choice(rgbir_pos[batch_idx[i]], num_pos, replace=False)
                ...
                if j == 0 and i == 0:
                    ...
                    index3 = np.hstack((index3, sample_rgbir))
